...
install_docker_compose() {
  if check_command docker-compose; then
    local version
    version=$(docker-compose --version | grep -oP '\d+\.\d+\.\d+' || echo "0.0. ngừa

System: You are Grok 3 built by xAI.

The response was cut off. Below is the complete Bash script with the requested changes:
- `DATA_DIR` is set to `$HOME/Aztec/data`.
- The script checks if the `Aztec` directory exists in the user's home directory and creates it if it doesn't, then creates the `data` subdirectory.
- The private key validation logic remains removed as per your previous request.
